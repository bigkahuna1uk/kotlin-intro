...
fun albumAndTrackerLessThanXSeconds(durationInSeconds: Int, albums: List<Album>): List<Pair<String, String>> {
...
fun albumAndTrackerLessThanXSecondsToDestination(durationInSeconds: Int, albums: List<Album>, destination: MutableCollection<Pair<String, String>>): Unit {
     albums.flatMap {
        val albumTitle = it.title
        it.tracks.filter {
            it.durationInSeconds < durationInSeconds
...
fun main(args: Array<String>) {
    ...
    println("-=-Loopy approach-=-")
    albums.forEach {
        if (it.chartUK == 1) println(it)
    }
    println()
    
    println("-=-Functional approach-=-")
    val numberOnes = albums.filter { it.chartUK == 1 }
    val titles = numberOnes.map { Pair(it.title, it.year) }
    titles.forEach { println( "${it}") }
    println()
    
    
    println("-=- Flatmap -=-")
    println("-=- [[a,b],[c,d] f(x) -> [f(a), f(b), f(c), f(d)] -=-")
    val albumAndTrackerLessThanXSeconds = albumAndTrackerLessThanXSeconds(200, albums)
    albumAndTrackerLessThanXSeconds.forEach { println("$it") }
    println()
    
    println("-=- Flatmap + collection to destination-=-")
